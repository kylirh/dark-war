...
export class Renderer {
  ...
  public centerOnPlayer(
    ...
    const basePlayerX =
      "worldX" in player && typeof player.worldX === "number"
        ? offsetX + player.worldX
        : offsetX + player.x * CELL_CONFIG.w + CELL_CONFIG.w / 2;
    const basePlayerY =
      "worldY" in player && typeof player.worldY === "number"
        ? offsetY + player.worldY
        : offsetY + player.y * CELL_CONFIG.h + CELL_CONFIG.h / 2;

    // Calculate player's screen position (at configured scale)
    const playerScreenX = basePlayerX * this.scale;
    const playerScreenY = basePlayerY * this.scale;
