...
export class Game {
  ...
  public reset(depth: number = 0): void {
    ...
    const dungeonLevel = outside ? null : this.createDungeonLevel(depth);
    const dungeon = outside ?? dungeonLevel!;
  ...
  /**
   * Generate a fresh bounded dungeon level in full, up front, with the entry
