...
describe("computeStateDelta / applyStateDelta", () => {
  ...
  it("round-trips exploredByPlayer growth", () => {
    const next = baseState();
    next.exploredByPlayer = { p1: [0, 1, 2] };
    roundTrip(baseState(), next);
  });

  it("round-trips exploredByPlayer removals", () => {
    const next = baseState();
    next.exploredByPlayer = { p1: [0] };
    roundTrip(baseState(), next);
  });

  it("round-trips exploredByPlayer new player", () => {
    const next = baseState();
    next.exploredByPlayer = { p1: [0, 1], p2: [3, 4] };
    roundTrip(baseState(), next);
  });

  it("round-trips exploredByPlayer deleted player", () => {
    const next = baseState();
