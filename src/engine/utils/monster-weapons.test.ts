...
describe("isAdaptiveWeaponMonster", () => {
  it("returns true for adaptive monsters", () => {
    expect(isAdaptiveWeaponMonster(makeMonster({ type: MonsterType.ZYTH }))).toBe(true);
    expect(isAdaptiveWeaponMonster(makeMonster({ type: MonsterType.TERRORIST_COLLABORATOR }))).toBe(true);
  });

  it("returns false for non-adaptive monsters", () => {
    expect(isAdaptiveWeaponMonster(makeMonster({ type: MonsterType.MUTANT }))).toBe(false);
    expect(isAdaptiveWeaponMonster(makeMonster({ type: MonsterType.SKULKER }))).toBe(false);
  });
});
...
describe("equippedMonsterWeaponItem", () => {
  it("returns the equipped weapon for adaptive monsters", () => {
    const zyth = makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.LASER_PISTOL });
...
describe("equippedMonsterWeaponType", () => {
  it("returns the correct WeaponType for the equipped weapon item", () => {
    expect(equippedMonsterWeaponType(makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.LASER_PISTOL }))).toBe(WeaponType.LASER);
    expect(equippedMonsterWeaponType(makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.GYROJET_SMG }))).toBe(WeaponType.SMG);
    expect(equippedMonsterWeaponType(makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.VIBRA_SWORD }))).toBe(WeaponType.MELEE);
    expect(equippedMonsterWeaponType(makeMonster({ type: MonsterType.MUTANT }))).toBe(WeaponType.MELEE);
  });
});

describe("monsterCanUseEquippedWeapon", () => {
  it("returns true if monster has enough ammo for pistol", () => {
    const monster = makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.PISTOL, bullets: 1 });
    expect(monsterCanUseEquippedWeapon(monster)).toBe(true);
  });

  it("returns false if monster does not have enough ammo for pistol", () => {
    const monster = makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.PISTOL, bullets: 0 });
    expect(monsterCanUseEquippedWeapon(monster)).toBe(false);
  });

  it("returns true if monster has enough ammo for shotgun", () => {
    const monster = makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.GYROJET_SHOTGUN, bullets: MONSTER_SHOTGUN_AMMO_COST });
    expect(monsterCanUseEquippedWeapon(monster)).toBe(true);
  });

  it("returns false if monster does not have enough ammo for shotgun", () => {
    const monster = makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.GYROJET_SHOTGUN, bullets: MONSTER_SHOTGUN_AMMO_COST - 1 });
    expect(monsterCanUseEquippedWeapon(monster)).toBe(false);
  });

  it("returns true if monster has enough charge for laser", () => {
    const monster = makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.LASER_PISTOL, laserCharge: MONSTER_LASER_SHOT_COST });
    expect(monsterCanUseEquippedWeapon(monster)).toBe(true);
  });

  it("returns false if monster does not have enough charge for laser", () => {
    const monster = makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.LASER_PISTOL, laserCharge: MONSTER_LASER_SHOT_COST - 1 });
    expect(monsterCanUseEquippedWeapon(monster)).toBe(false);
  });

  it("returns true for melee weapons regardless of ammo", () => {
    const monster = makeMonster({ type: MonsterType.ZYTH, equippedWeapon: ItemType.VIBRA_SWORD, bullets: 0, laserCharge: 0 });
    expect(monsterCanUseEquippedWeapon(monster)).toBe(true);
  });
});
...
describe("monsterMeleeDamage", () => {
  it("returns max of monster dmg and weapon floor damage", () => {
    ...
    expect(monsterMeleeDamage({ ...weakMonster, equippedWeapon: ItemType.VIBRA_SWORD } as Monster)).toBe(7);
    expect(monsterMeleeDamage({ ...weakMonster, equippedWeapon: ItemType.MACRO_METAL_SWORD } as Monster)).toBe(5);
    expect(monsterMeleeDamage({ ...weakMonster, equippedWeapon: ItemType.BUTCHER_KNIFE } as Monster)).toBe(3);

    const strongMonster = makeMonster({ type: MonsterType.ZYTH, dmg: 10 });
    expect(monsterMeleeDamage({ ...strongMonster, equippedWeapon: ItemType.VIBRA_SWORD } as Monster)).toBe(10);
    expect(monsterMeleeDamage({ ...strongMonster, equippedWeapon: ItemType.MACRO_METAL_SWORD } as Monster)).toBe(10);
    expect(monsterMeleeDamage({ ...strongMonster, equippedWeapon: ItemType.BUTCHER_KNIFE } as Monster)).toBe(10);
  });

  it("returns base monster dmg for other items", () => {
    const monster = makeMonster({ type: MonsterType.ZYTH, dmg: 4, equippedWeapon: ItemType.PISTOL });
