...
- **What we give up:** compiler-enforced completeness of the registries. `ITEM_DEFS` and `MONSTER_DEFS` are annotated `Record<ItemType, ItemDef>` (`item-defs.ts:50`) and `Record<MonsterType, MonsterDef>` (`monster-defs.ts:70`), so today a member added to the enum without a matching entry is a build error — adding `ItemType.PROBE_ONLY` with no entry yields `error TS2741: Property '[ItemType.PROBE_ONLY]' is missing ... but required in type 'Record<ItemType, ItemDef>'`. Once the registry is compiled data rather than a checked literal, that annotation cannot hold: if the key type is derived from the data it is circular, and if it stays a hand-written enum the two can drift. The compiler must then re-implement this check, and it is not optional work — it is replacing a guarantee `tsc` currently gives for free. This trade is the mirror image of the one the ADR is recommending: relational invariants move _into_ the build step, and completeness invariants move _out of_ the type system.
