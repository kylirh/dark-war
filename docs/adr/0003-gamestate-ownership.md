...
**The case for this:** The TypeScript compiler would structurally enforce that no simulation system can directly mutate the arrays. It eliminates the root cause of the silent state-drift bugs recorded in `.jules/scribe.md` and allows us to delete the defensive invariant tests.
...
- **What gets better:** The compiler enforces lifecycle boundaries. We can delete defensive invariant tests (like `entity-lookup.test.ts` and the `state.players` sync test). State-sync regressions caused by array mutation become impossible.
