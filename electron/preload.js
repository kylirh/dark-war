...
contextBridge.exposeInMainWorld("native", {
  ...
  saveWriteSlot: (slot, dataStr) =>
    ipcRenderer.invoke("save:write-slot", slot, dataStr),
  ...
  onServerExited: (callback) => {
    ipcRenderer.on("server:exited", (_e, data) => callback(data));
  },

  // ── Multiplayer: LAN discovery ─────────────────────────────────────────────
  discoveryStartBroadcast: (info) =>
    ipcRenderer.invoke("discovery:start-broadcast", info),
  discoveryUpdateBroadcast: (info) =>
    ipcRenderer.invoke("discovery:update-broadcast", info),
