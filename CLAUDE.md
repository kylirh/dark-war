...
**Addressed worlds:** the server simulates one `LevelWorld` (its own `Game` + `Physics`) per stable `WorldAddress`, shared by everyone on that plane. Players migrate independently through typed portals such as stairs, cave mouths, and holes; only the acting player moves. Empty planes are frozen but retained. Migration carries the player entity (HP/inventory) via `Game.detachPlayer`/`attachExistingPlayer` and forces a keyframe so the client rebaselines on the destination plane.
...
The terrain and world foundation is complete. Read these before editing its code:
...
The implemented design uses compositional typed-array tile layers on 2D WorldPlanes,
...
Do not reintroduce scalar runtime maps or editor IDs as gameplay state.
