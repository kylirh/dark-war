...
**Action:** Replaced the tear-down-and-rebuild strategy with object reuse. A `calloutDisplays` map in `WorldCalloutLayer` now tracks active callout containers by their unique `callout.id`. During a render tick, the system first sweeps for any stale containers (callouts that have expired) and only destroys those. Then, for remaining active callouts, it fetches the existing container and simply updates its transform and alpha parameters. A 500-frame test running the same 20 callouts with this pooled approach reduced the CPU time from ~4212ms to ~31ms (a 135x speedup).
