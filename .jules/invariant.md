...
## 2026-09-12 - Fix silent drift in entity array ordering during delta compression

**What was found:** The delta compression logic (`src/net/state-delta.ts`) reconstructed entities and players from a diff using `applyById`, which stored them in a JavaScript `Map`. It then used `Array.from(byId.values())` to rebuild the list. Because `Map` iteration yields elements in insertion order, applying a delta that included upsertions or mid-array removals would silently alter the original entity array ordering, dropping the server-side order. This violated the serialization invariant that `applyStateDelta` must yield exactly the state of `state_full`.

**Action:** Added an `orderChanged` detection check in `diffById` that checks if the ID sequence changed. If so, `computeStateDelta` now populates an explicit `entityOrder` and `playerOrder` array of string IDs into the `StateDelta`. `applyById` now takes this explicit `order` parameter and uses it to reconstruct the exact array sequence instead of relying on `Map` iteration order.

**Prevention:** When synchronizing entity state arrays over the network, do not rely on implicit mapping or insertion-ordered collections if array order matters for determinism. Always explicitly diff and serialize the canonical ordering if it can change during mutations.
