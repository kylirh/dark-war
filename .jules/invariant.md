...
**Action:** Claude Code pushed a correction to the PR: compare the `next` order against the order that `applyById` *would* naturally reconstruct, and only send the explicit list when they actually differ. Also added tests to cover player reordering and ensure the list is omitted when redundant.
